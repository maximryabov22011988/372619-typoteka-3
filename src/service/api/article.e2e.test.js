...
describe(`API changes existent article`, () => {
  ...
  test(`Status code 200`, () => expect(response.statusCode).toBe(HttpCode.OK));
  test(`Article is really changed`, () => request(app).get(`/articles/2`).expect((res) => expect(res.body.title).toBe(`Новый заголовок`))
  );
});
...
test(`API refuses to delete non-existent comment`, async () => {
  const app = await createAPI();
  return request(app).delete(`/articles/1/comments/UNKNOWN`).expect(HttpCode.NOT_FOUND);
});
